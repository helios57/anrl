...
GO
IF OBJECT_ID(N'[dbo].[FK_FlightPoint4D]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[PointSet_Point4D] DROP CONSTRAINT [FK_FlightPoint4D];
GO
...
GO
IF OBJECT_ID(N'[dbo].[FK_Point4D_inherits_Point]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[PointSet_Point4D] DROP CONSTRAINT [FK_Point4D_inherits_Point];
GO
...
GO
IF OBJECT_ID(N'[dbo].[PointSet_Point4D]', 'U') IS NOT NULL
    DROP TABLE [dbo].[PointSet_Point4D];
GO
...
-- Creating table 'PointSet_Point4D'
CREATE TABLE [dbo].[PointSet_Point4D] (
...
-- Creating primary key on [Id] in table 'PointSet_Point4D'
ALTER TABLE [dbo].[PointSet_Point4D]
ADD CONSTRAINT [PK_PointSet_Point4D]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
-- Creating foreign key on [Flight_Id] in table 'PointSet_Point4D'
ALTER TABLE [dbo].[PointSet_Point4D]
ADD CONSTRAINT [FK_FlightPoint4D]
    FOREIGN KEY ([Flight_Id])
    REFERENCES [dbo].[FlightSet]
        ([Id])
    ON DELETE CASCADE ON UPDATE NO ACTION;
...
CREATE INDEX [IX_FK_FlightPoint4D]
ON [dbo].[PointSet_Point4D]
    ([Flight_Id]);
GO
...
-- Creating foreign key on [Id] in table 'PointSet_Point4D'
ALTER TABLE [dbo].[PointSet_Point4D]
ADD CONSTRAINT [FK_Point4D_inherits_Point]
    FOREIGN KEY ([Id])
    REFERENCES [dbo].[PointSet]
        ([Id])
    ON DELETE CASCADE ON UPDATE NO ACTION;
