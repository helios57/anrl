...
namespace PFA.ANR.BusinessLayer
{
    ...
    public class CompetitorGroupCollection : AnrObject, ICollection
    {
        ...
        public CompetitorGroup this[Guid competitorGroupId]
        {
            get
            {
                foreach (CompetitorGroup competitorGroup in items)
	            {
            		 if (competitorGroup.Id == competitorGroupId)
                     {
        ...
        public CompetitorGroup this[int competitorGroupNumber]
        {
            get
            {
                foreach (CompetitorGroup group in this)
                {
                    if (group.GroupNumber == competitorGroupNumber)
                    {
                        return group;
                    }
                }
